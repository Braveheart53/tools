...
# %%%% 0.2.14: Large-scan support; companion to engine 0.3.18.
# Date: 2026-09-09
#              No new controls -- the single "Also convert image files"
#              checkbox is unchanged -- but the engine underneath it now
#              handles the archival scans that made the first version
#              unusable on a real archive:
#                * Pillow's ~89 MP "decompression bomb" refusal is lifted,
#                  which was failing every 240+ MP large-format scan.
#                * Downsampling happens before mode conversion, and
#                  greyscale stays greyscale: peak memory per worker fell
#                  from 1564 MB to 603 MB, and to 76 MB where libvips is
#                  installed.
#                * libvips is used when present, so a four-worker run over
#                  large scans needs ~300 MB rather than ~2.4 GB.
#              The image tooltip now names the pyvips option, since on a
#              scan archive it is the difference between a run that fits in
#              memory and one that does not.
# %%%% 0.2.13: Image conversion, and author metadata.
# Date: 2026-09-09
#              NEW single checkbox on Files & Output, "Also convert image
#              files", wired to the engine's --images.  One option, as
#              asked: it covers JPEG, PNG, TIFF, BMP, GIF, WebP, JPEG 2000,
#              AVIF, ICO, Netpbm, Targa, PCX, EPS and Windows metafiles.
#              Each image becomes a PDF named after its source file and
#              appears in the combined PDF's bookmarks under that same
#              name; a multi-page TIFF becomes a multi-page PDF with one
#              bookmark per page.
#                * The "found" label now counts images separately --
#                  "12 files - 5 DWG, 7 DXF" when seven are JPEGs would be
#                  a quietly wrong label.
#                * The tooltip states the .emf/.wmf Windows-only caveat,
#                  because those render through the Windows GDI.
#              Author metadata added to the module header and to the About
#              dialog, and Spyder # %% cell markers used throughout.
# %%%% 0.2.12: "Never use AutoCAD" strategy.
# Date: 2026-09-09
#              NEW fifth engine strategy, exposing the engine's
#              --strategy no-autocad.  AutoCAD is the only engine that
#              starts a GUI application, takes a licence seat, can raise a
#              modal dialog, and may hand each finished PDF to a viewer;
#              when the free decoder already converts almost every sheet,
#              this avoids all of it and reports the rest as failures to
#              re-run separately.
...
class ConversionWorker(QtCore.QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Once per run, HERE.  The CLI gets this from run_batch();
            # this window never calls run_batch(), so until 0.2.17 a GUI
            # operator was never told the DWG decoder was four years out
            # of date -- and an old decoder fails drawings in pass 1 that
            # then cost full AutoCAD time in the serial pass.
            try:
                warning = engine.check_decoder()
            except Exception:                      # noqa: BLE001
                warning = None
            if warning:
                self.message.emit("WARNING: " + warning)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _tab_files(self) -> QtWidgets.QWidget:
        ...
        self.images_cb = QtWidgets.QCheckBox("Also convert image files")
        self.images_cb.setToolTip(
            "Also convert any image files found in the selected folders:\n"
            "JPEG, PNG, TIFF, BMP, GIF, WebP, JPEG 2000, AVIF, ICO, Netpbm,\n"
            "Targa, PCX, EPS, and Windows metafiles (.emf/.wmf).\n"
            "\n"
            "Each becomes a PDF beside the drawings, named after the source\n"
            "file, and appears in the combined PDF's bookmarks under that\n"
            "same name. A multi-page TIFF or GIF becomes a multi-page PDF\n"
            "with one bookmark per page.\n"
            "\n"
            "Off by default: a drawing folder usually also holds thumbnails,\n"
            "logos and scanned correspondence that nobody wants in a drawing\n"
            "set.\n"
            "\n"
            ".emf and .wmf need Windows -- they are rendered through the\n"
            "Windows GDI, and cannot be rasterised on Linux or macOS.\n"
            "\n"
            "LARGE SCANS: if this archive holds large-format scans (240+\n"
            "megapixels is normal for an E-size sheet at archival dpi),\n"
            "install pyvips -- 'pip install pyvips pyvips-binary'. Measured\n"
            "on a 244 MP scan it needs 76 MB per worker instead of 603 MB,\n"
            "and runs twice as fast. Everything still works without it."
        )
        self.images_cb.stateChanged.connect(self._rescan)
        ...
        for cb in (self.recursive_cb, self.dxf_cb, self.images_cb,
                   self.skip_cb):
            hb.addWidget(cb)
        hb.addStretch(1)
        f.addRow("", hb)
    ...
    def _tab_engine(self) -> QtWidgets.QWidget:
        ...
        self.strategy_combo.setToolTip(
            ...
            "\n"
            "'Never use AutoCAD' is worth knowing about. AutoCAD is the only\n"
            "engine that starts a GUI application, checks out a licence seat,\n"
            "can raise a modal dialog, and may hand each finished PDF to a\n"
            "viewer. When the free decoder already converts almost every\n"
            "sheet, paying all of that for the last one per cent is often the\n"
            "wrong trade -- the drawings it cannot decode are then reported\n"
            "as failures you can re-run separately."
        )
    ...
    def _about(self) -> None:
        QtWidgets.QMessageBox.about(
            ...
            "Batch conversion of 2D AutoCAD drawings, and optionally image "
            "files, to PDF.<br><br>"
            ...
            "per-drawing fallback, or exclusively if you ask.<br><br>"
            "<b>Author:</b> %s<br>"
            "<b>Email:</b> <a href=\"mailto:%s\">%s</a><br>"
            "<b>Work phone:</b> %s<br><br>"
            "<small>MIT-licensed tooling around GPL-3 LibreDWG "
            "(see vendor/libredwg-win64/COPYING-LibreDWG-GPL3.txt).</small>"
            % (engine.__revision__, __revision__,
               __author__, __email__, __email__, __phone__)
        )
    ...
    def scan_inputs(self) -> None:
        ...
        n_img = sum(1 for p in self._sources
                    if p.suffix.lower() in engine.IMAGE_SUFFIXES)
        n_dxf = len(self._sources) - n_dwg - n_img
        # Count images separately rather than folding them into the DXF
        # total: "12 files - 5 DWG, 7 DXF" when seven of them are JPEGs
        # would be a quietly wrong label.
        summary = "%d file(s) - %d DWG, %d DXF" % (
            len(self._sources), n_dwg, n_dxf)
        if n_img:
            summary += ", %d image(s)" % n_img
        self.found_label.setText(summary)
        self._refresh_preview()

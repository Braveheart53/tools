...
def check_engine_fix_coverage() -> None:
    ...
    # 0.4.3 -- the decoder warning that run_batch() does for the CLI.
    chk("the GUI calls check_decoder() itself (0.4.3)",
        "check_decoder()" in src)
    chk("its message reaches the log pane, not only the logger",
        'self.message.emit("WARNING: " + warning)' in src)
    ...
    # 0.4.0 -- and all of it must survive the trip to a spawned worker.
    chk("more than one call site passes the settings snapshot",
        src.count("engine.backend_settings()") >= 2)
...
def run_test() -> int:
    ...
    chk("no-AutoCAD strategy offered", "no-autocad" in strategies)
    chk("all five strategies offered", len(strategies) == 5)
    ...
    chk("two folders scanned into 8 drawings",
        "8 file(s)" in w.found_label.text())
    chk("image checkbox present and off by default",
        hasattr(w, "images_cb") and not w.images_cb.isChecked())
    chk("no image count shown while images are off",
        "image(s)" not in w.found_label.text())
